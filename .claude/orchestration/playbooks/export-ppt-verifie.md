...
```json
{
  ...
  "etapes": [
    ...
    },
    {
      ...
      "contrat": {
        "type": "deterministe",
        "critere": "SEUIL OBJECTIF SUR LE CHEMIN DU DIFF (durci le 2026-07-28 : formulée « SI le SWOT n'est pas au RDV », la condition était auto-jugée par celui qui venait d'écrire le code — donc jamais déclenchée, 0 invocation en 6 jours et 4 commits deck). SI le diff touche `app/services/pptx_export/slides_diagnostic.py` (porte `_slide_swot` depuis le découpage a3ca545) : charger la skill `swot-matrix` AVANT de dessiner (matrice 2×2 réelle, axes Interne/Externe × Favorable/Défavorable explicites). SI le diff touche `app/services/pptx_export/slides_trajectoire.py` (porte `_slide_matrice_effort_valeur`) : charger la skill `priority-matrix` AVANT de dessiner (quadrants teintés porteurs de sens + bulles colorées par axe, JAMAIS un scatter Excel natif). SI une slide est dessinée ou retouchée, quelle qu'elle soit : consulter `deck-design-library` AVANT (choisir la forme depuis l'intention, pas l'inverse). Rattachement 2026-07-23 (supervision multi-projets) : ces skills existaient depuis le 2026-07-22 sans être câblées dans aucune chaîne."
      },
      "checkpoint": false
    },
    {
